...
var confess = {
        ...
        if (!this.processArgs(cliConfig, [
            {
                ...
                oneof: ['performance', 'appcache', 'cssproperties', 'top10']
            }, {
                name: 'configFile',
                def: 'config.json',
                req: false,
    ...
    top10: {
        ...
        onInitialized: function (page, config) {
            ...
            }else if (config.userAgent == "android"){
                page.evaluate(function () {
                    (function () {
                        window.screen = {
                            width: 360,
        ...
        onLoadFinished: function (page, config, status) {
            ...
            resources.forEach(function (resource) {
                //Calculate top 10 slowest
                if (top10Slowest.length < 10){
                    top10Slowest.push(resource);
                } else if (resource.duration > top10Slowest[9].duration){
                    top10Slowest.pop();
                    top10Slowest.push(resource);
                }
                top10Slowest.sort(sortByDuration);

                //Calculate top 10 largest
                if (top10Largest.length < 10){
                    top10Largest.push(resource);
                } else if (resource.size > top10Largest[9].size){
                    top10Largest.pop();
                    top10Largest.push(resource);
                }
                top10Largest.sort(sortBySize);
            });

            var ths = this;

             console.log('Top 10 Slowest: ');
            top10Slowest.forEach(function (resource) {
               if( resource.size == undefined)
                    resource.size = 0;
               console.log(ths.pad(resource.duration,6) + ' ms; ' + ths.pad(resource.size,7) + ' b; ' + ths.truncate(resource.url));
            });
            console.log('Top 10 Largest: ');
            top10Largest.forEach(function (resource) {
                if( resource.size == undefined)
                    resource.size = 0;
                console.log(ths.pad(resource.duration,6) + ' ms; ' + ths.pad(resource.size,7) + ' b; ' + ths.truncate(resource.url));
            });
